...
const semanticChecks = [
  ["variable declarations", 'num x ~ 1; text y ~ "false"'],
  //   ["complex array types", "task f(x: [[[int?]]?]) {}"],
  ["increment and decrement", "num x ~ 10 x -~ 1 x +~ 1"],
  ["initialize with empty array", "stream a ~ []"],
  ["type declaration", "struct S {f: (int)->boolean? g: string}"], // like interface in Typescript?
  ["assign arrays", "stream a ~ [] stream b ~ [1] a~b b~a"],
  ["assign to array element", "stream a ~ [1,2,3]; a[1]~100;"],
  //   ["initialize with empty optional", "let a = no int;"], No 'void' in our language
  ["short return", "task f() return"],
  ["long return", "task f() return true"],
  //   ["assign optionals", "let a = no int;let b=some 1;a=b;b=a;"], No optionals in our language
  ["return in nested if", "task f() if (true) return"],
  ["break in nested if", "loop() if (true) dry"],
  ["long if", "if (true) outflow(1) else outflow(3)"],
  ["elsif", "if (true) outflow(1) else if (true) outflow(0) else outflow(3)"],
  ["for over collection", "loop(length([2,3,5])) outflow(1)"],
  ["for in range", "loop(9) outflow(0)"],
  ["repeat", "loop(3) num a = 1 outflow(a)"], // don't need this
  ...
  // ["||", "outflow(true||1<2||false||!true);"],
  ["&&", "print(true&&1<2&&false&&true);"],
  // ["&&", "print(true&&1<2&&false&&!true);"],
  // ["bit ops", "print((1&2)|(9^3));"],
  ["relations", 'outflow(1<=2 && "x">"y" && 3.5<1.2);'],
  ["ok to == arrays", "outflow([1]==[5,8]);"],
  ["ok to != arrays", "outflow([1]!=[5,8]);"],
